...
def test_ethane_bonds_properly_predicted():
    atoms = ase.io.read("tests/ethane.xyz")
    bonds, bond_types, _, _, _ = guess_bonds(
        atoms,
        np.ones(len(atoms)),
        [20, 20, 20],
        [90, 90, 90],
    ...
    assert (
        np.testing.assert_array_equal(unique_bond_types, expected_unique_bond_types)
        == None
    )
...
def test_ethane_bond_orders_correct():
    ...
    expected_bond_count = OrderedDict(
        {"0": 1, "1": 4, "2": 1, "3": 1, "4": 4, "5": 1, "6": 1, "7": 1}
    )
...
def test_periodic_bonding():
    atoms = Atoms(
        "CCCC", positions=[(0.1, 5, 5), (1.1, 5, 5), (2.1, 5, 5), (3.1, 5, 5)]
    )
    bonds, _, _, _, _ = guess_bonds(
        atoms,
        np.ones(len(atoms)),
        [4, 10, 10],
        [90, 90, 90],
