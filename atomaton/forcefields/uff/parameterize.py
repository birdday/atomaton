...
def search_for_aromatic_carbons(atoms, all_dihedrals, uff_symbols, ring_tol=0.1):
    ...
    for i in range(len(all_dihedrals)):
        for j in range(i + 1, len(all_dihedrals)):
            ...
            if (
                end_points_d1 == end_points_d2
                and len(set((p0, p1, p2, p3))) == 4
                and set(atom_types) == {"C"}
            ):
                p0, p1, p2, p3 = (
                    atoms[p0].position,
                    atoms[p1].position,
...
def search_for_secondary_aromatics(atoms, bonds_present, uff_symbols):
    ...
    for atom in atoms:
        if atom.symbol == "C" or atom.symbol == "N" or atom.symbol == "O":
            bonded_atom_types = [
                uff_symbols[val]
                for bond in bonds_present[str(atom.index)]
                for val in bond
                if val != atom.index
            ]
...
def get_pair_potentials(atom_types):
    ...
    for atom_type in atom_types:
        lj_sigma = UFF_DATA[atom_type]["x1"] * (2 ** (-1.0 / 6.0))
...
def guess_bond_orders(bond_types):
    ...
    for bond_type in bond_types:
        ...
        if len(set(["H_", "F_", "Cl", "Br", "I_"]).intersection(set(bond_type))) != 0:
            ...
            bond_order_dict[key] = 1
        elif len(set(bond_type)) == 1 and set(bond_type).issubset(
            set(["C_2", "N_2", "O_2"])
        ):
            ...
            print(
                bond_type,
                ":Bond order not properly assigned. Using default value of 1.",
            )
            bond_order_dict[key] = 1
...
def get_angle_parameters(angle_types, angle_values, angle_tol=5, degrees=True):
    ...
    for angle_type in angle_types:
        ...
        # Linear Angles
        if (
            angle_values[angle_type][0] >= 180 - angle_tol
            and angle_values[angle_type][0] <= 180 + angle_tol
        ):
        ...
        # Trigonal Planar
            ...
            angle_values[angle_type][0] >= 120 - angle_tol
            and angle_values[angle_type][0] <= 120 + angle_tol
        ):
            lammps_angle_type = "cosine/periodic"
            n, theta_o = 3, 120
        ...
        # Square Planar or Octahedral
            ...
            angle_values[angle_type][0] >= 90 - angle_tol
            and angle_values[angle_type][0] <= 90 + angle_tol
        ):
            lammps_angle_type = "cosine/periodic"
            n, theta_o = 4, 90
        ...
        # r_ik
        r_ik = np.sqrt(
            r_ij**2 + r_jk**2 - 2 * r_ij * r_jk * np.cos(np.deg2rad(theta_o))
        )
        ...
        k_ijk = (
            ...
            * (r_ij * r_jk)
            * (
                (3 * r_ij * r_jk) * (1 - np.cos(np.deg2rad(theta_o)) ** 2)
                - (r_ik**2 * np.cos(np.deg2rad(theta_o)))
            )
        )
