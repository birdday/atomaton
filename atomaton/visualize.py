...
def draw_unit_cell(
    cell_lengths, cell_angles, cell_r=0.15, color=(0.4, 0.4, 0.4), opacity=1.0
):
    unit_cell_corners = [
        [0, 0, 0],
        [1, 0, 0],
    ...
    corner_points.mlab_source.dataset.lines = np.array(
        (
            (0, 1),
            (0, 2),
            (0, 3),
...
def view_structure(
    ...
    atom_sf, bond_r, cell_r, bond_color, cell_color = (
        objects["atom_sf"],
        objects["bond_r"],
        objects["cell_r"],
        objects["bond_color"],
    ...
    if bond_energies != None:
        ...
        _, binned_bonds = bin_data(
            bond_energies, bonds, num_bins=bond_bins, val_range=e_range
        )
    ...
    if parallel == True and distance != None:
        ...
        xx, yy, zz = np.array(
            [[x, y, z] for x in xvals for y in yvals for z in zvals]
        ).transpose()
        pts = mlab.points3d(
            xx, yy, zz, resolution=6, scale_factor=0, color=(1.0, 1.0, 1.0)
        )
